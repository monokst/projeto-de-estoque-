...
DELIMITER //
CREATE PROCEDURE insere_produto (n VARCHAR(45), e INT)
BEGIN
	INSERT INTO produto (nome, estoque_minimo)
    VALUES (n, e);
    SELECT * FROM produto;
...
DELIMITER //
CREATE PROCEDURE delete_produto_por_id (id INT)
BEGIN
...
CALL insere_produto ('Produto A', 100);
CALL delete_produto_por_id (11);
...
BEGIN
	...
	END;
    START TRANSACTION;
    IF EXISTS (SELECT 1 FROM categoria WHERE idC = id OR nome = n) 
    THEN
